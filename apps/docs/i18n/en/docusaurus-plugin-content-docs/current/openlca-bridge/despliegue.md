...
Before deploying the bridge service between DTAgro and OpenLCA, you need to have downloaded:

- the repository ([https://github.com/quercus-uex/Ventum-OpenLCA-Service](https://github.com/quercus-uex/Ventum-OpenLCA-Service)) (Capture ACV).
...
ACV_COMPARE_BASE_URL="http://acv-compare-backend:3000"  # ACV Compare base URL
...
:::note
The `.env.example` file contains all default values required for a standard deployment. You only need to modify `ACV_COMPARE_BASE_URL` if the ACV Compare URL differs from the default configuration.
:::

## Shared Network with ACV Compare

If you want to communicate this service with **ACV Compare**, you need to create the shared `olca` network:

```bash
docker network create olca
```

Both services must be connected to this network so ACV Compare can receive calculation results and the frontend can route calculation requests through the reverse proxy to the bridge service.
