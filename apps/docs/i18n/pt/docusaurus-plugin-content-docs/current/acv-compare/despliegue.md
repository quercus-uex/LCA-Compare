...
CAPTURE_ACV_EMAIL="email@example.com"    # Email para autenticação no DTAgro (extração massiva)
CAPTURE_ACV_PASSWORD="P@ssw0rd"          # Palavra-passe para autenticação no DTAgro (extração massiva)
...
| `acv-compare-backend` | Construída a partir de `apps/server/Dockerfile` | 8080→3000 | `prod` |
| `acv-compare-frontend` | Construída a partir de `apps/web/Dockerfile` | 80→80 | `prod` |
...
- **`olca`**: rede externa partilhada com o serviço Capture ACV. Deve ser criada manualmente:
...
| Rota | Destino |
|---|---|
| `/api/` | `acv-compare-backend:3000` (API REST, removendo o prefixo `/api`) |
...
4. Executa as migrações pendentes com `pnpm --filter server prisma:migrate:deploy` dentro do contentor `acv-compare-backend`.
