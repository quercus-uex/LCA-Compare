...
**Capture ACV** é um microsserviço **Python 3.12+** com **FastAPI** que atua como ponte entre a plataforma **DTAgro** e o motor de cálculo de Avaliação do Ciclo de Vida **openLCA**, com base de dados em formato **.zolca**. Recebe os dados de uma cultura a partir do DTAgro, executa o cálculo de impacto ambiental em openLCA através do seu servidor IPC e envia o resultado para [ACV Compare](https://github.com/quercus-uex/Ventum-ACV-Visualizer) para visualização e comparação.
...
POST /capture-acv    →    ACVService.execute()
                         ...
                         ├── build_final_result()
                         └── send_result_to_app() →  POST para ACV Compare (best-effort, erros registados)
```
...
- **Receção de dados de cultura** a partir do DTAgro através do endpoint `POST /capture-acv`.
...
- **Envio de resultados** para o ACV Compare para persistência, visualização e comparação (best-effort).
...
| `ACV_COMPARE_BASE_URL` | URL base do serviço ACV Compare para envio de resultados |
...
O fluxo de dados é: DTAgro → Capture ACV (cálculo) → ACV Compare (persistência e visualização).
